...
family_single = total_data['FamilySize'].map(lambda f: 1 if f==1 else 0)
family_small = total_data['FamilySize'].map(lambda f: 1 if 2<=f<=4 else 0)
family_large = total_data['FamilySize'].map(lambda f: 1 if f>=5 else 0)
...
# Embarked
embarked = pd.get_dummies(total_data['Embarked'],prefix='Embarked')

#======================================
# Feature Extraction/Redunduncy Removal
#======================================
full_X = pd.concat([sex, age, family_single, family_small, family_large, cabin], axis=1)
...
# Grid Searching and Fitting
Cs = np.logspace(0,2,20)
prediction_model_lr = GridSearchCV(estimator=LogisticRegression(solver='liblinear'),cv=stra_k_fold,param_grid=dict(C=Cs),iid=False,n_jobs=-1)
