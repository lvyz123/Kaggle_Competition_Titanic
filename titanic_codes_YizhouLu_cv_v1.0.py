...
sex = pd.get_dummies(total_data['Sex'],prefix='Sex')
...
total_data['Ticket'] = total_data['Ticket'].map(lambda t: t.split()[0].replace('/','').replace('.','').strip())
total_data['Ticket'] = total_data['Ticket'].map(lambda t: t[:-1].upper() if (not t.isdigit()) and t[-1].isdigit() else t.upper())
total_data['Ticket'] = total_data['Ticket'].map(lambda t: len(t) if t.isdigit() else t)
...
print(prediction_model_rfc.best_score_, prediction_model_rfc.best_estimator_.n_estimators)
